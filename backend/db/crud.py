...
async def get_user_by_email(db: AsyncSession, email: str) -> UserRegistration | None:
    result = await db.execute(select(UserRegistration).where(UserRegistration.email == email))
    return result.scalar_one_or_none()


async def create_user_registration(db: AsyncSession, email: str, name: str, tenant_id: str) -> UserRegistration:
    user = UserRegistration(email=email, name=name, tenant_id=tenant_id)
    db.add(user)
    await db.commit()
